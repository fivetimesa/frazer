    ...
    for(Node n: group1)
      for(Spring s: n.getConnections()) 
        if(diagonals.contains(s))
          s.changeRestLength(dds1);
        else
          s.changeRestLength(ds1);
      
    for(Node n: group2)
      for(Spring s: n.getConnections()) 
        if(diagonals.contains(s))
          s.changeRestLength(dds2);
        else
          s.changeRestLength(ds2);
